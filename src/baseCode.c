...
void free(char *ap) {
	 ...
	 * (Assumes that 0 is not a valid address for allocation. Also, i_alloc() must be
	 * called prior to using either free() or malloc(); otherwise, the free list will
	 * be null.) */
	...
	/*	Otherwise, current free-space head is lower in memory. Walk down free-space
	 * list looking for the block being returned. If the next pointer points past
	 * the block, make a new entry and link it. If next pointer plus its size
	 * points to the block, form one contiguous block. */
	nxt = frhd;
	for (nxt = frhd; nxt && nxt < f; prev = nxt, nxt = nxt->ptr) {
		if (nxt + nxt->size == f) {
			nxt->size += f->size; /* They're contiguous. */
...
void* malloc(int nbytes) /* bytes to allocate */
{
	...
	/* Change that divide to a shift (for speed) only if the compiler doesn't do
	 * it for you, you don't require portability, and you know that sizeof(HEADER)
	 * is a power of two. Allocate the space requested plus space for the header
	 * of the block. Search the free-space queue for a block that's large enough.
	 * If block is larger than needed, break into two pieces and allocate the
	 * portion higher up in memory. Otherwise, just allocate the entire block. */
